...
public class PawnShopInterface : Singleton<PawnShopInterface>
{
	...
	private void ChangeAnger(float change)
	{
		...
		if (NPCAnger >= 1f)
		{
			...
			else
			{
				...
				PawnShopNPC.Behaviour.CombatBehaviour.SetTargetAndEnable_Server(Player.Local.NetworkObject);
			}
	...
	private float GetItemValue(ItemInstance item)
	{
		...
		int hashCode = (((item.Name[0] + item.Name.Length > 1) ? item.Name[1].ToString() : "A") + NetworkSingleton<TimeManager>.Instance.DayIndex).GetHashCode();
