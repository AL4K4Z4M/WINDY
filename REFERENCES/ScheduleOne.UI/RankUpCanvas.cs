...
public class RankUpCanvas : MonoBehaviour, IPostSleepEvent
{
	...
	public void Start()
	{
		...
		instance2.onSleepStart = (Action)Delegate.Combine(instance2.onSleepStart, new Action(QueuePostSleepEvent));
	}
	...
	public void StartEvent()
	{
		...
		UnlockedItemsCanvasGroup.alpha = 0f;
		...
		IEnumerator Routine()
		{
			...
			foreach (Tuple<FullRank, int, int> progress in progressDisplays)
			{
				...
				float lerpTime = Mathf.Lerp(0.5f, 2f, (float)(endXP - startXP) / (float)NetworkSingleton<LevelManager>.Instance.GetXPForTier(oldRank.Rank));
