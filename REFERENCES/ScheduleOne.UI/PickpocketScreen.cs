...
public class PickpocketScreen : Singleton<PickpocketScreen>
{
	...
	{
		base.Start();
		Singleton<ItemUIManager>.Instance.AddRaycaster(Canvas.GetComponent<GraphicRaycaster>());
	}
	...
	public void Open(NPC _npc)
	{
		...
		for (int j = 0; j < Slots.Length; j++)
		{
			...
			float num = itemSlotUI.assignedSlot.ItemInstance.GetMonetaryValue() * (itemSlotUI.assignedSlot.ItemInstance.Definition as StorableItemDefinition).PickpocketDifficultyMultiplier;
			float x = Mathf.Lerp(GreenAreaMaxWidth, GreenAreaMinWidth, Mathf.Pow(Mathf.Clamp01(num / ValueDivisor), 0.3f)) / npc.Inventory.PickpocketDifficultyMultiplier;
		...
		ActionsContainer.gameObject.SetActive(value: true);
	...
	public void SetSlotLocked(int index, bool locked)
	{
		...
		if (num == 0)
		{
			ActionsContainer.gameObject.SetActive(value: false);
		}
	}
